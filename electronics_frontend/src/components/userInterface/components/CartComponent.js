...
export default function CartComponent(props)
{
    ...
    const handleQtyChangle=(product,value)=>{
      if(value<=0)
      {
          dispatch({type:"REMOVE_PRODUCT",payload:[product.productdetailsid]})
      }
      else 
      {
          product['qty']=value
    ...
    return data.map((item)=>{
      ...
      />
                ...
                <div  style={{marginLeft:'4%',borderRadius:8,marginTop:'-2%',display:'flex',justifyContent:'center',background:'#fff',fontSize:matches?10:15}}>
                  <PlusMinusComponent value={item?.qty} onChange={(value)=>handleQtyChangle(item,value)} screen='cart' />
                </div>
                </div> 
                <br></br>
